...
def search_nodes(key, value)
    if key == "recipe"
        ...
        if not results.include?(node) and node.run_list.expand(node.chef_environment).recipes.include?("ceph::#{value}")
            results.push(node)
        end
    elsif key == "role"
...
def get_mon_nodes
    ...
    results.map! { |x| x['hostname'] == node['hostname'] ? node : x }
    if not results.include?(node) and node.run_list.roles.include?('ceph-mon')
        results.push(node)
    end
    return results.sort! { |a, b| a['hostname'] <=> b['hostname'] }
end
...
def get_bootstrap_node
    results = search(:node, "role:ceph-admin-bootstrap AND chef_environment:#{node.chef_environment}")
    raise 'There is not exactly one ceph-admin-bootstrap node found.' if results.size != 1
    results.first
end
